...
- OpenFOAM: blockMesh/checkMesh, signed `phi` patch integrals, density conversion to mass flow, initial equation-residual histories, developed Poiseuille velocity/pressure-gradient agreement, and explicit hexahedral connectivity. Fields and patch integrals must come from the final measured solver iteration; older output cannot be mixed into that run.

The output directory contains `execution-report.json`, `verification-report.json`, and each case's runtime log, source project, export/result manifests, result package and solver output. Failures retain these files. Each result package carries the export's exact project/case/input fingerprint/run identity. Template executions and analytic benchmark comparisons are separate checks.
