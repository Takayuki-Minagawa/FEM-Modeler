...
`run.sh` は実行開始時に前回の結果を削除し、失敗時には非0の終了コードを持つmanifestを残します。環境準備やメッシュ検査の失敗でも前回の成功パッケージは残しません。保存済みプロジェクトを読み込む際にも、結果・メッシュのソルバーと指紋、フィールドの参照IDを再検査します。

OpenFOAMの代表寸法は実メッシュから求めます。2Dは前後面の合計面積の半分を `A` として `h = sqrt(A/Ncells)`、3Dは全セル体積を `V` として `h = cbrt(V/Ncells)` です。値と定義をmanifestへ記録し、2Dの人工的な厚さを収束比較から除きます。
