...
- **テンプレート** - strict export可能な5種の参照入力から開始（外部ソルバによる数値ベンチマーク検証は別途必要）
- **インポート** - 元単位を明示するSTLファイル (ASCII/バイナリ)、プロジェクトJSON
...
- **Undo / Redo** - 主要なIR編集の取り消し・やり直しに対応（現状はスナップショット履歴方式）
- **自動保存 / データ復元** - IndexedDB に作業内容を自動保存 (モデルサイズに応じた動的ディレイ) し、次回起動時に復元可能
- **エクスポート履歴** - エクスポート結果 (成功/警告/エラー) を下部パネルで一覧表示
- **ファイルバリデーション / マイグレーション** - 読込時にZodスキーマで検証し、旧バージョンのファイルも自動マイグレーション
- **ダーク/ライトモード** - テーマ切替対応
- **多言語対応** - 日本語 (デフォルト) / 英語
- **CAE補助** - メッシュ事前推定、ResultIR取込、3メッシュGCI、細長比・Re・Bi・Fo評価
...
テンプレートを選ぶと形状・材料・断面・境界条件・荷重・解析ケースが自動生成されます。いずれもアプリ内のstrict exporter contract testを通る参照入力ですが、外部solver runtimeによる基準解との照合済みベンチマークではありません。
...
├── results           # 再取込した共通ResultIRと保存則チェック
...
- メッシュ画面の要素数は形状寸法と設定値からの事前推定です。Jacobian、aspect ratio、skewness、負体積はメッシュ生成後に測定してください。
- ResultIR取込はprovenance付き数値CSV/JSON manifestの共通要約、構造解析の力の釣合い、solver process statusと明示された収束情報を対象とします。provenance不足や非数値列の除外はpartial扱いです。熱収支、CFD mass balance、XDMF/VTK/OpenFOAM fieldの完全なcontour表示は未実装です。
